...
$stmt = $pdo->prepare("SELECT notice_posts.*, users.username FROM notice_posts JOIN users ON notice_posts.author_id = users.id WHERE notice_posts.id = ?");
$stmt->execute([$id]);
$post = $stmt->fetch();

/* 댓글 목록을 작성자 이름과 같이 오래된 순으로 가져옴 */
$stmt2 = $pdo->prepare("SELECT notice_comments.*, users.username FROM notice_comments JOIN users ON notice_comments.author_id = users.id WHERE notice_comments.post_id = ? ORDER BY notice_comments.created_at ASC");
$stmt2->execute([$id]);
$comments = $stmt2->fetchAll();

/* 첨부파일 목록 가져오기 */
$stmt3 = $pdo->prepare("SELECT * FROM notice_attachments WHERE post_id = ?");
$stmt3->execute([$id]);
